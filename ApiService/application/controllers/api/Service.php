...
class Service extends REST_Controller {
 ...
 public function SelectQuo_post() {
   $data = $this->post();
   $data = $this->ServiceModel->SelectQuotation($data['quoId']);
   if(!empty($data)){
     //set the response and exit
     $this->response($data, REST_Controller::HTTP_OK);
   }else{
 ...
 public function quotationSubmitUpdate_post() {
   $data = $this->post();
   $substrQuo = substr($data['Forms']['SelectCustomer']['quoNo'], 0, 11);
   $countQuo = $this->db->where('quoNo LIKE','%'.$substrQuo.'%')->get('quotation')->num_rows()+1;
   $fusionQuo = $substrQuo."-".$countQuo;
   $quotation = array(
     'quoNo' => $fusionQuo,
     'quoCompany' => $data['Forms']['SelectCustomer']['customerCompany'],
     'quoCustomerId' => $data['Forms']['SelectCustomer']['customerId'],
   ...
   $this->ServiceModel->SaveQuotationProduct($Product);
   $updateOldQuo = array(
    'quoId' => $data['Forms']['SelectCustomer']['quoId'],
    'quoStatus' => 4,
    'quoDate' => $data['Forms']['SelectCustomer']['quoDate'],
    );
   $this->db->where('quoId',$updateOldQuo['quoId'])->update('quotation',$updateOldQuo);
   //check if the user data exists
   if(!empty($data)){
     //set the response and exit
     $this->response($data, REST_Controller::HTTP_OK);
