...
class ProductImport extends REST_Controller {
  ...
  public function SelectProfile_get() {

    $sessionid = $this->uri->segment(4);
    $dataProfile = $this->ProductImportModel->SelectProfile($sessionid);

    //check if the user data exists
    if(!empty($dataProfile)){
      //set the response and exit
      $this->response($dataProfile, REST_Controller::HTTP_OK);
    }else{
      //set the response and exit
      $this->response([
        'status' => FALSE,
